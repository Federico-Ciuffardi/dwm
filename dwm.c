...
enum { SchemeNorm, SchemeSel, SchemeNormTab, SchemeSelTab, SchemeSelAlt}; /* color schemes */
...
drawbar(Monitor *m)
{
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
		if (uline && (ulineall || m->tagset[m->seltags] & 1 << i)) /* if there are conflicts, just move these lines directly underneath both 'drw_setscheme' and 'drw_text' :) */
...
togglebar(const Arg *arg)
{
  ...
  updatebarpos(selmon);
  resizebarwin(selmon);
  if (showsystray && selmon == systraytomon(NULL) ) {
    XWindowChanges wc;
    if (!selmon->showbar)
      wc.y = -bh;
    else {
...
zoom(const Arg *arg)
{
  ...
  Client *c = selmon->sel;
  if(!c) return;
  if (arg && arg->i == 0 && ((selmon->lt[selmon->sellt] != &layouts[TALL] && selmon->lt[selmon->sellt] != &layouts[DECK]) || c->isfloating || c == cm)){
    rotate(&((Arg){.i=-1}));
    return;
  }
  ...
  if (arg && (!cm || !nexttiled(cm->next)
    ...
    selmon->prevhfocus = selmon->sel->snext;
    if(cm && selmon->prevhfocus == cm) selmon->prevhfocus = selmon->prevhfocus->snext;
